...
function checkParams() {
  if (argv.help) {
    ...
    if (argv.key && typeof argv.key !== "string") {
      ...
      console.error(
        "Please provide either district-id or pincode by appending --district=<DISTRICT-ID> or --pin=<PINCODE> \nRefer documentation for more details"
      );
      return;
      ...
      console.error(
        "Please provide an interval greater than or equal to 1 minutes"
      );
      return;
      ...
      !argv.dose ||
      (argv.dose && argv.dose !== 1 && argv.dose !== 2)
    ) {
      console.error(
        "Please mention if your require first dose or second dose by passing --dose=1 or --dose=2 \n"
      );
      return;
    } else if (
      (argv.vaccine && typeof argv.vaccine !== "string") ||
      (argv.vaccine &&
        argv.vaccine.toLowerCase() !== "covishield" &&
        argv.vaccine.toLowerCase() !== "covaxin" &&
        argv.vaccine.toLowerCase() !== "sputnik")
    ) {
      console.error(
        "Please provide vaccine param as COVAXIN or COVISHIELD or SPUTNIK"
      );
      return;
    } else if (
      argv["keep-alive"] &&
      typeof argv["keep-alive"] !== "string" &&
      argv["keep-alive"].toLowerCase() !== "true" &&
      argv["keep-alive"].toLowerCase() !== "false"
    ) {
      console.error("Please set keep-alive param as true or false");
      return;
    } else if (
      argv["centre-type"] &&
      typeof argv["centre-type"] !== "string" &&
      argv["centre-type"].toLowerCase() !== "free" &&
      argv["centre-type"].toLowerCase() !== "paid" &&
      argv["centre-type"].toLowerCase() !== "all"
    ) {
      console.error("Please set centre-type param as free, paid or all");
      return;
    } else {
      const params = {
        vaccine:
          argv.vaccine === "spuntik" ? argv.vaccine + " v" : argv.vaccine, // vaccine = COVISHIELD , COVAXIN, SPUTNIK
        ...
        keepAlive: argv["keep-alive"]
          ? argv["keep-alive"].toLowerCase() === "true"
          : defaultKeepAlive,
        centreType: argv["centre-type"]
          ? argv["centre-type"].toLowerCase()
          : defaultCentreType,
      };
...
function pingCowin({
  ...
  let url = pin
    ? `${baseUrl}calendarByPin?pincode=${pin}&date=${date}`
    : `${baseUrl}calendarByDistrict?district_id=${districtId}&date=${date}`;

  const ageLimit = age >= 18 && age < 45 ? 18 : 45;

  axios
    .get(url, { headers: { "User-Agent": sampleUserAgent } })
    .then((result) => {
      ...
      if (centers.length) {
        centers.forEach((center) => {
          if (centreType !== center.fee_type || centreType !== "all") {
            return;
          }
          center.sessions.forEach((session) => {
            if (
              ...
              if (
                vaccine &&
                vaccine.toLowerCase() !== session.vaccine.toLowerCase()
              ) {
                return;
              }
